...
function updateMapWithUsers(users) {
    ...
    users.forEach(u => {
        ...
        m.on('popupopen', function() {
            document.getElementById('popup-chat-' + u.user_id)?.addEventListener('click', function() { startChat(u.user_id, u.username); });
            document.getElementById('popup-profile-' + u.user_id)?.addEventListener('click', function() { showUserProfile(u.user_id, u.username); });
            document.getElementById('popup-route-' + u.user_id)?.addEventListener('click', function() { window.open('https://www.google.com/maps/dir/?api=1&destination=' + u.lat + ',' + u.lng, '_blank'); });
            document.getElementById('popup-report-' + u.user_id)?.addEventListener('click', function() {
                const reason = prompt("Motif du signalement :");
                if (reason) reportUser(u.user_id, reason);
            });
            document.getElementById('popup-block-' + u.user_id)?.addEventListener('click', function() {
