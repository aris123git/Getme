...
class SMSParser:
    def __init__(self):
        ...
        self.patterns = {
            'orange_money': {
                'sender_patterns': ['OM', 'Orange Money', '+226'],
                'regex': r'Reçu de ([+\d]+).*?(?P<amount>[\d\s]+)\s*F\s*CFA',
                'amount_clean': lambda x: int(x.replace(' ', ''))
            },
            'wave': {
                'sender_patterns': ['Wave', 'WAVE'],
                'regex': r'vous avez reçu (?P<amount>[\d]+)\s*F\s*CFA de (?P<sender>[+\d]+)',
                'amount_clean': lambda x: int(x)
            },
            'moov_money': {
                'sender_patterns': ['Moov Money', 'Moov'],
                'regex': r'Virement de (?P<amount>[\d]+)\s*FCFA reçu de (?P<sender>[+\d]+)',
                'amount_clean': lambda x: int(x)
            }
        }
    
    def detect_operator(self, sms_text, sender):
        """Détecte l'opérateur en fonction du texte et de l'expéditeur"""
        for operator, config in self.patterns.items():
            for pattern in config['sender_patterns']:
                if pattern.lower() in sms_text.lower() or pattern in sender:
                    return operator
        return 'unknown'
    ...
    def parse(self, sms_text, sender_number):
        ...
        # Extraire l'expéditeur (si disponible dans le match)
